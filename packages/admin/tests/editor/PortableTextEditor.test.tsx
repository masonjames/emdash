...
describe("plugin block helpers", () => {
	...
	it("falls back to initial_value only when the current value is undefined", () => {
		const selectField = {
		...
		expect(_resolvePluginBlockFieldValue(selectField, undefined)).toBe("inline");
		expect(_resolvePluginBlockFieldValue(selectField, "")).toBe("");
	});
});
