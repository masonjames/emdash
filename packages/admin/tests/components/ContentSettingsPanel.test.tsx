...
describe("ContentSettingsPanel", () => {
	...
	it("lets editors correct the publication date", async () => {
		const onUpdatePublishedAt = vi.fn();
		const screen = await render(
			<ContentSettingsPanel
				{...makePanelProps({
					item: makeItem({ publishedAt: "2025-01-15T10:30:00Z" }),
					status: "published",
					isLive: true,
					onUpdatePublishedAt,
				})}
		...
		await input.fill("2025-02-01T12:00");
		await screen.getByRole("button", { name: "Update date" }).click();

		expect(onUpdatePublishedAt).toHaveBeenCalledWith("2025-02-01T12:00:00.000Z");
	});

	it("does not expose publication-date editing below the editor role", async () => {
		const screen = await render(
			<ContentSettingsPanel
				{...makePanelProps({
					item: makeItem({ publishedAt: "2025-01-15T10:30:00Z" }),
					currentUser: AUTHOR_ROLE,
					onUpdatePublishedAt: vi.fn(),
				})}
			/>,
		);

		expect(screen.getByLabelText("Published on").query()).toBeNull();
	});
});
