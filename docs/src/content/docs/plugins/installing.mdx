...
<Aside type="tip">
  Some plugin families ship as a split pair: a trusted package for full rendering features, plus a marketplace-safe companion for admin-managed settings. `emdash-social-sharing` is the reference example — its marketplace companion can manage defaults, while the trusted npm package provides Astro rendering and Portable Text block registration.
</Aside>
...
- Are required for features that depend on trusted-only surfaces such as `componentsEntry` or `admin.portableTextBlocks`
