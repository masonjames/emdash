...
5. **Feature restrictions**
   ...
   When you need both trusted rendering and marketplace installation, use a split-variant pattern: keep the full rendering package trusted, and ship a separate marketplace-safe companion for settings or workflow UI. The `emdash-social-sharing` plugin family uses this pattern.
