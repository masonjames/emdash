...
<Aside type="caution">
  `emdash plugin bundle` and `emdash plugin publish` are for **standard / sandboxed** plugins only. If your plugin depends on trusted-only surfaces like `componentsEntry`, `admin.entry`, or `admin.portableTextBlocks`, publish the trusted package through npm and create a separate marketplace-safe companion if you need admin-installable sandboxed behavior.
</Aside>
...
- **Trusted-only features rejected** — bundled plugins cannot declare `admin.entry` or `admin.portableTextBlocks`
