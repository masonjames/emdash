...
Public page hooks let plugins contribute to request-time discovery behavior plus the `<head>` and `<body>` of rendered pages. Templates opt in using the `<EmDashHead>`, `<EmDashBodyStart>`, and `<EmDashBodyEnd>` components from `emdash/ui`.

### `site:discovery`

Contributes request-time discovery configuration for core-owned public endpoints like `/sitemap.xml` and `robots.txt`. Plugins contribute settings, not public routes. Works in both trusted and sandboxed modes.

```typescript
"site:discovery": async (_event, ctx) => {
  const enabled = await ctx.kv.get<boolean>("settings:sitemap-enabled");
...
**Returns:** `SiteDiscoveryContribution | null`
...
First contributed value wins for each field. Current core defaults are equivalent to:
...
Contributes typed metadata to `<head>` — document titles, meta tags, OpenGraph properties, canonical/alternate links, and JSON-LD structured data. Works in both trusted and sandboxed modes.
...
`archive` is explicit template-provided page identity for collection indexes, taxonomy term archives, and search pages. Use it when plugins need archive-aware defaults. Do not overload `pageType` or infer archive type from paths.

`structuredData` is explicit template-provided page context. It is the intended path for schema modules like FAQ, HowTo, Product, and Review. Plugins should not infer those payloads by scraping rendered HTML.
...
| ---------- | -------------------------------------------------- | ------------------------------------------------ |
| `title`    | `<title>...</title>`                               | singleton; first contribution wins               |
...
| `site:discovery`       | Discovery route request   | Contribution or `null`        |
